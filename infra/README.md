...
|---|---|---|---|
| `DB_URL` | JDBC connection string, for a Java service reading it directly | trade-api, trade-executor | Both Java services' `application.yml` |
| `DB_USERNAME`, `DB_PASSWORD` | Discrete Spring datasource credentials | trade-api, trade-executor | Both Java services' `application.yml` |
...
| `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER` | Discrete database parameters | trade-api, trade-executor | Both Java services' `application.yml` |
| `KAFKA_BOOTSTRAP_SERVERS` | Broker address inside the compose network, `kafka:29092` | trade-api, trade-executor | Both Java services' `application.yml` |
| `KAFKA_CONSUMER_GROUP` | Explicit consumer group id | trade-executor (`trade-executor`) | `services/trade-executor/src/main/resources/application.yml` |
| `FAUXNANCE_BASE_URL`, `FAUXNANCE_API_KEY` | Passed through from `.env`, never generated by compose | trade-executor | `services/trade-executor/src/main/resources/application.yml` |
| `POLL_INTERVAL_SECONDS` | Seconds between market-data poll cycles | trade-executor | `services/trade-executor/src/main/resources/application.yml` |
| `JWT_SECRET` | Shared HS256 signing secret. Must be at least 32 characters | trade-api, auth-service, auth-stub | Trade API and Auth configuration |
